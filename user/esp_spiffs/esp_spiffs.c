...
#include "esp_spiffs/spiffs/spiffs.h"
...
static s32_t esp_spiffs_erase(u32_t addr, u32_t size)
{
    ...
    for (i = 0; i < sectors; i++)
    {
        if (!spiflash_erase_sector(addr + (SPI_FLASH_SEC_SIZE * i)))
        {
            return SPIFFS_ERR_INTERNAL;
        }
    }
...
int _open_r(struct _reent *r, const char *pathname, int flags, int mode)
{
    ...
    if (flags & O_CREAT)
        spiffs_flags |= SPIFFS_CREAT;
    if (flags & O_APPEND)
        spiffs_flags |= SPIFFS_APPEND;
    if (flags & O_TRUNC)
        spiffs_flags |= SPIFFS_TRUNC;
    if (flags & O_RDONLY)
        spiffs_flags |= SPIFFS_RDONLY;
    if (flags & O_WRONLY)
        spiffs_flags |= SPIFFS_WRONLY;
    if (flags & O_EXCL)
        spiffs_flags |= SPIFFS_EXCL;
